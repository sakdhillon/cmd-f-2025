...
export const getMed = async (userID, medName) => {
    try {
        const res = await axios.get(`http://localhost:8080/meds/edit?userID=${userID}&med=${medName}`);
...
export const addMed = async ({ inputData }) => {
    try {
      const res = await axios.post('http://localhost:8080/meds/add', inputData, {
        headers: {
...
export const editMed = async ({ inputData }) => {
    try {
      const res = await axios.post('http://localhost:8080/meds/edit', inputData);
      return res.data;
    } catch (err) {
